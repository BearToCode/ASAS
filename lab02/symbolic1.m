...
V = m * g * l * cos(theta);
D = 1/2 * c * dx ^ 2 +1/2 * b * dtheta ^ 2;

Q_x = f;
Q_theta = 0;

f_x = diff(diff(T, dx), t) - diff(T, x) + diff(V, x) + diff(D, dx) - Q_x;
f_theta = diff(diff(T, dtheta), t) - diff(T, theta) + diff(V, theta) + diff(D, dtheta) - Q_theta;
...
theta0 = pi; % initial pendulum angle
