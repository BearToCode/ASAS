function sys = sloshing_undamped(pendulums, params)
    % SLOSHING_UNDAMPED calculates the state space model for the sloshing
    % pendulums model without damping.
    %
    % sys = sloshing_undamped(pendulums, params) returns the state space model
    % for the sloshing pendulums model without damping.
    %
    % Input arguments:
    ...
    for i = 1:pendulums.n
        A = blkdiag(A, [0 1; -pendulums.w_n(i) ^ 2 0]);
    end
